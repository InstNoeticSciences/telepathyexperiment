...
include("lib/tt_sms.php");

// -- tt project includes --
include("lib/tt_twilio_util.php");      // twilio-related utilities
include("lib/tt_experiment.php");       // the experiment object
...
/*================================================== tt experiment: start */
if($_SESSION['user']) {
    $is_coordinator = $_SESSION['user']->is_coordinator;
    $smarty->assign("is_coordinator", $is_coordinator);
} // end if
/*================================================== tt experiment: end   */
...
/*================================================== tt experiment: start */
    else if(!is_invited($_POST['email'])) $smarty->assign("error_email", err_not_invited);
/*================================================== tt experiment: end   */
    else {
        if($_POST['visible']) $visible = 1; else $visible = 0;
        $db = new database;
        $db->dblink();
        $id = $db->db_insert("users", "time, username, pass, email, visible, is_coordinator", time().", '{$_POST['username']}', '{$_POST['pass1']}', '{$_POST['email']}', $visible, 'Y'");
        if($id) {
            $rec = $db->get_rec("users", "*", "id=$id");
            $_SESSION['user'] = new user($rec);

            //confirmation email
            $msg = str_replace("#username", $_SESSION['user']->username, confirmation_tt_mail);
            $msg = str_replace("#link", $base_href."welcome/".encrypt($_SESSION['user']->username."###".$_SESSION['user']->pass), $msg);
            mail($_SESSION['user']->email, confirmation_tt_subject, $msg, "From: Admin<".CONTACT_MAIL.">");
...
/*================================================== tt experiment: start */
            // remove from the invited users list
            del_invited($_POST['email']);
/*================================================== tt experiment: end   */
    ...
    if($how_many != 0) {
        ...
        if($how_many != 0){
            ...
            if(is_array($recs)) foreach($recs as $rec){
                $u = new user($rec);
                $msg = str_replace("#username", $u->username, confirmation_tt_mail);
                $msg = str_replace("#link", $base_href."welcome/".encrypt($u->username."###".$u->pass), $msg);
                mail($u->email, confirmation_tt_subject, $msg, "From: Admin<".CONTACT_MAIL.">");
...
/*================================================== tt experiment: start */
        if($_SESSION['user']) {
            ...
            if($_POST['my_download']) {
                download_results("initiator='{$_SESSION['user']->username}'");
            } // end if

            if($_POST['all_download']) {
                download_results();
            } // end if
        } // end if
/*================================================== tt experiment:   end */
...
/*================================================== tt experiment: start */
    // restore the user instance
    $user = $_SESSION['user'];
    $new_experiment = $_SESSION['new_experiment'];
    $new_trial = $_SESSION['new_trial'];
    $step_number = $_SESSION['step_number'];

    // first page load initialisation
    if($_POST['page_load'] <= 0) {
        ...
        for($i = 0; $i < $num_friends; $i++) {
            $_POST['friend_list'][$_POST['friend_idx']] = $friends[$i]->username;
            $_POST['friend_idx']++;
        } // end for
    ...
    // initialise the button statuses
    button_status($smarty, 'transfer', ($_POST['friend_idx'] > 0 ? "true" : "false"));
    button_status($smarty, 'rejected', ($_POST['part_idx'] > 0 ? "true" : "false"));
    button_status($smarty, 'start', ($_POST['part_idx'] >= 4 ? "true" : "false"));
    button_status($smarty, 'choose', ($_POST['part_idx'] >= 4 ? "true" : "false"));
    button_status($smarty, 'restart', "false");
    button_status($smarty, 'next', "false");

    // initialise the step-dependent screen texts
    set_experiment_texts($smarty, $step_number);
    
    // transfer participants
    if($_POST['transfer']) {
        status_message(INFORMATION, transfer_participants().participants_added);
        button_status($smarty, 'transfer', (($_POST['friend_idx'] > 0) && ($_POST['part_idx'] < 4)? "true" : "false"));
        button_status($smarty, 'rejected', ($_POST['part_idx'] > 0 ? "true" : "false"));
        button_status($smarty, 'start', ($_POST['part_idx'] >= 4 ? "true" : "false"));
        button_status($smarty, 'choose', "false");
        button_status($smarty, 'restart', "false");
        button_status($smarty, 'next', "false");

        if($_POST['part_idx'] == 4) {
            status_message(ACTION, click_start_the_trial_to_begin_the_trial);
        } // end if
    ...
    // reject participants
    if($_POST['reject']) {
        status_message(INFORMATION, reject_participants().participants_rejected);
        button_status($smarty, 'transfer', ($_POST['friend_idx'] > 0 ? "true" : "false"));
        button_status($smarty, 'rejected', ($_POST['part_idx'] > 0 ? "true" : "false"));
        button_status($smarty, 'start', ($_POST['part_idx'] >= 4 ? "true" : "false"));
        button_status($smarty, 'choose', "false");
    ...
    if($_POST['start']) {
        // must have exactly the right number of participants
        if($_POST['part_idx'] != MAX_PARTICIPANTS) {
            status_message(ERROR, you_must_select_exactly.MAX_PARTICIPANTS.participants);
            button_status($smarty, 'transfer', ($_POST['friend_idx'] > 0 ? "true" : "false"));
            button_status($smarty, 'rejected', ($_POST['part_idx'] > 0 ? "true" : "false"));
            button_status($smarty, 'start', ($_POST['part_idx'] >= 4 ? "true" : "false"));
            button_status($smarty, 'choose', "false");
            ...
            // must explicitly set the texts for transition steps
            set_experiment_texts($smarty, $step_number);
            
            button_status($smarty, 'transfer', "false");
            button_status($smarty, 'rejected', "false");
            button_status($smarty, 'start', "false");
            button_status($smarty, 'choose', "false");
            button_status($smarty, 'restart', "false");
            button_status($smarty, 'next', "false");

            // new experiment or new trial
            if($new_trial == "true") {
                $my_experiment = $_SESSION['my_experiment'];
                $my_experiment->next_trial();
            } else {
                unset($my_experiment);
                $my_experiment = new experiment($_POST['part_list'], MAX_PARTICIPANTS, $user->username, $user->phone);
            } // end if

            $smarty->assign('experiment_number', '('.$my_experiment->get_experiment_id().'/');
            $smarty->assign('trial_number', $my_experiment->get_trial().')');
            
            status_message(INFORMATION, experiment.' '.$my_experiment->get_experiment_id().' '.
                                        trial.' '.$my_experiment->get_trial().' '.
                                        has_begun);

            // randomly select the caller
            $my_caller_name = $my_experiment->select_caller();

            // create the caller
            $call_rec = $db->get_rec("users", "*", "username='{$my_caller_name}'");
            $call_user = new user($call_rec);

            // create an entry in the call queue and generate an extension
            $extension = call_queue_create($my_experiment->get_experiment_id(),
                                           $my_experiment->get_trial(),
                                           $my_experiment->get_initiator_name(),
                                           $my_experiment->get_initiator_phone(),
                                           $my_caller_name);

            $_SESSION['extension'] = $extension;
            
            // send the caller details to the experiment
            $my_experiment->set_caller($call_user->username, $call_user->phone);
            status_message(INFORMATION, caller_has_been_selected);

            // inform the caller that they must call the initiator
            $msg = please_call.$my_experiment->get_initiator_name().on.TWILIO_PHONE.extension.$extension;

            if($my_experiment->notify_caller($call_user, $msg)) {
                button_status($smarty, 'choose', "true");

                // prompt to select one user from the participants list
                status_message(INFORMATION, caller_has_been_notified_via_sms);
                status_message(ACTION, please_choose_one_participant_from_the_list);
                
                // store the experiment instance for future reference
                $_SESSION['my_experiment'] = $my_experiment;
            } else {
                status_message(ERROR, sms_to_caller_failed);
            } // end if
        } // end if
    } // end if

    // initiator has selected a caller
    if($_POST['choose']) {
        // restore the experiment instance
        $my_experiment = $_SESSION['my_experiment'];
        ...
        if($_POST['sel_friends'][0] == null) {
            status_message(WARNING, you_have_not_selected_a_caller);
        } else {
            // set and remember the chosen caller
            $_POST['chosen_caller'] = $_POST['sel_friends'][0];
            $my_experiment->set_chosen_user($_POST['sel_friends'][0]);
            status_message(INFORMATION, you_have_chosen.$my_experiment->get_chosen_user().'.');

            // update the call queue
            caller_chosen($extension);

            // set and remember the actual caller
            $_POST['actual_caller'] = $my_experiment->get_caller_name();

            // check for a match
            if($my_experiment->caller_match()) {
                status_message(INFORMATION, you_have_correctly_chosen_user.$my_experiment->get_caller_name().'.');
            } else {
                status_message(INFORMATION, sorry_the_caller_was.$my_experiment->get_caller_name().'.');
            } // end if

            // close the trial
            if($my_experiment->close_trial()) {
                status_message(INFORMATION, that_is_the_end_of_this_trial);

                if($my_experiment->get_trial() >= MAX_TRIALS) {
                    // calculate results
                    $exp_hits = hits_for_experiment($my_experiment->get_experiment_id());
                    $exp_percent = percentage(MAX_TRIALS, $exp_hits, 2);

                    status_message(INFORMATION, that_is_the_end_of_this_experiment);
                    status_message(INFORMATION, you_scored.' '.$exp_hits.' '.hits.' ('.$exp_percent.'%).');
                    status_message(ACTION, to_start_a_new_experiment_click_new_experiment);
                    button_status($smarty, 'restart', "true");
                } else {
                    status_message(ACTION, to_run_another_trial_click_next_trial);
                    button_status($smarty, 'next', "true");
                } // end if
            } else {
                status_message(ERROR, there_was_a_problem_closing_this_trial);
            } // end if

            // disable all buttons and wait for another trial or restart
            button_status($smarty, 'transfer', "false");
            button_status($smarty, 'rejected', "false");
            button_status($smarty, 'start', "false");
            button_status($smarty, 'choose', "false");
    ...
    // another trial with the same participants
    if($_POST['next']) {
        // initialise the buttons
        button_status($smarty, 'transfer', "false");
        button_status($smarty, 'rejected', "false");
        button_status($smarty, 'start', "true");
        button_status($smarty, 'choose', "false");
        button_status($smarty, 'restart', "false");
        button_status($smarty, 'next', "false");

        // reinitialise the status box
        clear_status();
        status_message(ACTION, click_start_the_trial_to_begin_the_trial);

        // set the trial and experiment variables
        $new_trial = true;
        $new_experiment = false;
        $step_number = 2;

        // clear out the callers from the last trial
        $_POST['chosen_caller'] = null;
        $_POST['actual_caller'] = null;

        // reset the extension number
        $_SESSION['extension'] = 0;
    } // end if

    // start a new experiment
    if($_POST['restart']) {
        // reinitialise the participant list and the status box
        return_participants();
        clear_status();
        start_message();
        ...
        $smarty->assign('experiment_number', '');
        $smarty->assign('trial_number', '');

        // must explicitly set the texts for transition steps
        set_experiment_texts($smarty, $step_number);

        // reinitialise the buttons
        button_status($smarty, 'transfer', "true");
        button_status($smarty, 'rejected', "false");
        button_status($smarty, 'start', "false");
        button_status($smarty, 'choose', "false");
        button_status($smarty, 'restart', "false");
    ...
    $_SESSION['new_experiment'] = $new_experiment;
    $_SESSION['new_trial'] = $new_trial;
    $_SESSION['step_number'] = $step_number;
/*================================================== tt experiment: end   */
                ...
                if($_POST['save_life']){
                    ...
                    if($error)  $smarty->assign("error", $error);
                        ...
                        if($_POST['location'] !='') {
                            ...
                            $data = explode(",", geocode_curl(GOOGLE_MAPS_KEY, $_POST['location'], 0));
                        ...
                        $result = $db->db_update("users",
                                                 "age={$age},
                                                  bio='".urlencode(htmlspecialchars($_POST['bio']))."',
                                                  location='".urlencode($_POST['location'])."',
                                                  www='{$_POST['www']}',
                                                  interests='".urlencode(htmlspecialchars($_POST['interests']))."',
                                                  x={$x}, 
                                                  y={$y}",
                                                 "id={$_SESSION['user']->id}");
...
/*================================================== tt experiment: start */
                // populate the list of mobile phone carriers
                $result = $db->get_recs("carriers", "carrier_id");
                ...
                if($result) {
                    ...
                    foreach($recs as $rec) {
                        $carriers[] = $rec->carrier_id;
                    } // end foreach
                    
                    if(is_array($recs)) {
                        $smarty->assign('my_carrier', $_SESSION['user']->carrier_id);
        ...
        if($_POST['send_invitations']){
            if(is_array($_POST['user'])) foreach($_POST['user'] as $u) {
                ...
                if($data[2]!=0) {
                    ...
                    $msg = str_replace("#message_link", $base_href."message/".$last_update->id, invitation_tt_mail);
                    ...
                    $ok = mail($addr, invitation_tt_subject, $msg, "Content-Type: text/plain; charset=iso8859-1\nFrom: Admin<".CONTACT_MAIL.">");
                }
            }
            ...
            if(!$_POST['addresses']) $error = err_email_needed;
                ...
                if(is_array($addresses)) foreach($addresses as $addr){
                    ...
                    if($u) {
                        ...
                        $ok = mail($addr, invitation_tt_subject, $msg, "Content-Type: text/plain; charset=iso8859-1\nFrom: Admin<".CONTACT_MAIL.">");
/*================================================== tt experiment: start */
                        // add to the invitees list
                        add_invited($addr, $_SESSION['user']->username);
/*================================================== tt experiment: end   */
                    }
                }
                if($ok) $smarty->assign("ok", ok_invitations_sent_nolist);
...
/*================================================== tt experiment: start */
        reset_experiment();
/*================================================== tt experiment: end   */  
        $smarty->assign("mpp", mpp); break;
    case "terms_and_conditions": break;
    case "contact":
        if($_POST['send']){
            $_POST['subject'] = stripslashes($_POST['subject']);
            $_POST['message_content'] = stripslashes($_POST['message_content']);
