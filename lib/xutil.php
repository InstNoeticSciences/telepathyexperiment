...
// captcha keys
define("captcha_pub", "6LegqtkSAAAAAArqcmli9UhiU_gol4XOPb_HZQzb");
define("captcha_prv", "6LegqtkSAAAAACjJLgMfU9h-Ampa5isWzmHOT3AJ");
...
function generate_graph($user) {
    ...
    if(isset($user)) {
        ...
        if(!empty($results)) {
            ...
            $graph->setRange(MAX_TRIALS, 0);
            $graph->setGoalLine((1 / MAX_FRIENDS) * MAX_TRIALS);
...
function describe_results($user) {
    if(isset($user)) {
        ...
        if(!empty($results)) {
            ...
            $num_trials = $num_experiments * MAX_TRIALS;
            $num_hits = 0;
            ...
            $score = round(($num_hits / $num_trials * 100), 2);
            $chance = round((1 / MAX_FRIENDS * 100), 2);

            echo "<p class=\"info\">
                    So far you have conducted $num_trials trials over
                    $num_experiments experiments and have scored $num_hits hits.
                    ...
                    The numbers along the y-axis are the experiment numbers for
                    your experiments. The numbers on the x-axis show the number
