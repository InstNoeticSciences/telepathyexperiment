...
if($_POST['update'] || $_POST['enter']) {
    ...
    $_SESSION['user']->set_phone(reformat_phone_number($_POST['phone']));
    $_SESSION['user']->set_email($_POST['email']);
    $_SESSION['user']->set_age($_POST['age']);
    $_SESSION['user']->set_gender($_POST['gender']);
    $_SESSION['user']->set_group_name($_POST['group_name']);
    
    // first name validation
    switch(validate_name($_POST['first_name'], false)) {
        case BAD_LENGTH:
            $_SESSION['profile_result'] = "You have not entered a first name";
            $error = true;
            break;
        case BAD_CHARS:
            $_SESSION['profile_result'] = "First name must only contain letters and no space";
            $error = true;
            break;
        default:
            $_SESSION['profile_result'] = '';
...
/*
    ...
    switch(validate_name($_POST['group_name'])) {
        case BAD_LENGTH:
            $_SESSION['profile_result'] = "You have not entered a group name";
            $error = true;
            break;
        case BAD_CHARS:
            $_SESSION['profile_result'] = "Group name must only contain letters (no spaces)";
            $error = true;
            break;
        default:
            $_SESSION['profile_result'] = '';
...
*/
    ...
    if($_POST['update'] && !$error) {
        if($_SESSION['user']->db_update(UPDATE)) {
            ...
            $_SESSION['profile_result'] = 'could not save your changes';
            $error = true;
        } // end if
    } // end if
