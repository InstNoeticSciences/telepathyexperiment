...
if($user->username == $lu->username) $result = $db->get_recs("messages m LEFT JOIN groups_mes gm ON gm.group_id=m.group_id", "*,gm.group_title,gm.group_furl", "direct=0 and (msg like '".urlencode("@{$lu->username}")."%' or (reply in (select id from messages where direct=0 and user='{$user->username}') and user in (select username from users where id not in (select user from blocked_users where blocked_user={$user->id}))))", "time desc limit $limit offset $offset");
else $result = $db->get_recs("messages m LEFT JOIN groups_mes gm ON gm.group_id=m.group_id", "*,gm.group_title,gm.group_furl", "direct=0 and (msg like '".urlencode("@{$user->username}")."%' or reply in (select id from messages where direct=0 and user='{$user->username}'))", "time desc limit $limit offset $offset");
