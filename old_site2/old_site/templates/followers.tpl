{if $logged_user}
	<h1>{$user->count_followers()} {$followers_of1} {$user->username}</h1>
{else}
	<h1 class='header'>{$user->count_followers()} {$followers_of1} {$user->username}</h1>
{/if}
{$pages}
{foreach from=$followers item=i key=k}
