...
$count = $db->get_rec("messages", "count(*) as ile", "group_id=" . $_GET['group_id'], "time desc");	//all messages by this user
$msg_count = $count->ile;
...
$result = $db->get_recs("messages m LEFT JOIN groups_mes gm ON gm.group_id=m.group_id", "*,gm.group_title,gm.group_furl", "m.group_id=" . $_GET['group_id'], "time desc limit $limit offset $offset");	//all messages by this user
#var_dump($result);
...
if(is_array($messages)){
	foreach($messages as $k=>$m){
		echo "<div class='msg" . (($k%2==0) ? ' yellow' : '') . "'>";
