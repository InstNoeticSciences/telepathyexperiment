...
function set_informed_consent($phone, $status,$reset_num_calls = 0) {
	$resultValue = 1;
    $db = new database();
    $db->dblink();
    $db->db_update("tele_informedconsent", "status='{$status}'", "phone='{$phone}'");
	if ($reset_num_calls) $db->db_update("tele_informedconsent", "number_of_calls=0", "phone='{$phone}'");
}
...
function inc_call_informed_consent($phone) {
    ...
    $result = $db->get_recs("tele_informedconsent", "number_of_calls", "phone='{$phone}'");
    $recs = $db->fetch_objects($result);	
